...
class MHSA(nn.Module):
    def __init__(
        ...
        d_model,
        head_num,
        position_encoder: RotaryPositionEmbedding | None
    ) :
        super().__init__()
    ...
    def forward(
        ...
        # 分头行动
        Q, K, V = (
            einops.rearrange(X, "... seq_length (head_num d_head) -> ... head_num seq_length d_head", head_num=self.head_num)
            for X in (Q, K, V)
        )
        ...
        if self.position_encoder is not None:
            Q = self.position_encoder(Q, token_position_id)
            K = self.position_encoder(K, token_position_id)
        ...
        # 答：此时Q, K, V的形状是[batch_size, head_num, seq_length, d_head]
        ...
        # 多头合并
        x = einops.rearrange(x, "... head_num seq_length d_head -> ... seq_length (head_num d_head)").contiguous()
